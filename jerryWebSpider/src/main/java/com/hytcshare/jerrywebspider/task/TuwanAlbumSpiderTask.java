...
public class TuwanAlbumSpiderTask implements Runnable {
	private String welfareUrl;   //目标地址URL
	private int tuwanStart;      //开始编号
	private Integer endLine;     //结束编号
	...
	@Override
	public void run() {
		log.info("tuwan album spider start！,爬虫开始");
		try {
			...
			log.info("tuwan album spider task finish!，爬虫结束");
		}
	}
		...
		if (tuwanResult.contains("error_msg")) {
			...
			if (errorFlag == 0) {
				...
				if (tuwanAlbumImagesService.countByTitle(title) > 0) {
					log.info("************boolean={}***********", tuwanAlbumImagesService.countByTitle(title));
					return;
				}
